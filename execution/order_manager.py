...
class OrderManager:
    ...
    def place_order(
        ...
        retries     : int = 3,
        ordertag    : Optional[str] = None,   # SEBI algo-tagging — confirm requirement with broker
    ) -> str | None:
        """
        ...
        Args:
            ...
            ordertag: Optional tag (<=20 chars) to identify orders from this algo,
                      e.g. for SEBI algo-trading Algo ID tagging if your broker requires it.
        """
        if qty <= 0:
    ...
    def place_bracket_order(
        ...
        """
        Place a broker-side bracket order (Angel One ROBO/BO) so stop-loss and
        target are enforced on the exchange, not just by our own polling loop.

        NOTE: squareoff/stoploss are sent as POINTS away from entry_price (not
        absolute prices), per standard Indian-broker BO convention. Angel One's
        public docs don't give a full worked example for this order type —
        verify with a single small live order before relying on this in the
        main strategy loop.

        Falls back to a plain market order via place_order() in paper mode
        (paper fills are simulated locally regardless of bracket vs normal).
        """
        if qty <= 0:
            logger.warning(f"[OrderManager] Zero/negative qty for {symbol} — skipping.")
            return None
        ...
        if self.mode == "paper":
            return self._paper_place(symbol, token, exchange, transaction, qty,
                                      "LIMIT", entry_price, "BO",
                                      client_ref or str(uuid.uuid4())[:16])
        ...
        params = {
            ...
            "squareoff"        : str(squareoff_pts),
            "stoploss"         : str(stoploss_pts),
            "trailingStopLoss" : str(trailing_stoploss_points),
            "ordertag"         : (ordertag or "")[:20],
        }
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                if resp and resp.get("status"):
                    order_id = resp["data"]["orderid"]
                    self._pending[client_ref] = order_id
                    logger.info(f"[OrderManager] ✅ Placed BO {transaction} {qty}x{symbol} "
                                f"| SL={stoploss_pts}pts Target={squareoff_pts}pts | OrderID={order_id}")
                    return order_id
                logger.warning(f"[OrderManager] BO attempt {attempt} failed: {resp}")
            except Exception as exc:
                logger.warning(f"[OrderManager] BO exception attempt {attempt}: {exc}")
            time.sleep(2 ** attempt)

        logger.error(f"[OrderManager] ❌ BO order failed after {retries} attempts: {symbol}")
        return None
